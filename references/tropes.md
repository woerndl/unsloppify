...
Reference catalogue for the `unsloppify` skill. Scan drafted prose against every category before returning it.
...
Words like "fundamental", "crucial", "essential", "pivotal" that inflate ordinary statements. AI uses these to make normal observations sound important. If you remove the word and the sentence means the same thing, it was filler.
...
Stamping sequential labels like "Phase 1:", "Stage 2:", "Step 3:" onto sections or paragraphs. Humans describe what they're doing; AI announces a numbered march. Real writing flows by topic, not by countdown.
...
AI almost always leads with something positive before delivering criticism. It can't just say what's wrong. Real people (especially in technical contexts) just say what's wrong.
...
Claiming something is secret, unspoken, or overlooked when it adds nothing to the sentence. Phrases like "and this is the part nobody talks about" or "what most people miss" are only valid if you are genuinely pointing to something obscure, not well-reported, or when the text itself has the goal of making these differences known to the reader. Used as a generic aside, they create false insider drama. Don't use these punch-lines unless the content actually needs one.
...
**Acceptable use:** ONLY when the thing you are pointing to is genuinely not widely known and you can back that up.

### Clichéd Idioms

Stock phrases and idioms that no one actually uses in normal speech or writing. AI reaches for these because they sound authoritative or vivid, but the effect is rigid and unnatural. Avoid forensic-drama vocabulary, overused business idioms, and any phrase that sounds like it belongs either to a thriller synopsis or to the CEO's email asking everyone to "move the needle to the maximum".
...
AI puts casualness in quotation marks to perform relatability. Real casual writing is just casual; it doesn't need to frame itself.
